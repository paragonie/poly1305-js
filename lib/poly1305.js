...
module.exports = class Poly1305
{
     ...
     *
     * @return {module.Poly1305}
     */
    clone()
    {
        let clone = new Poly1305(this.key);
    ...
    async blocks(message, bytes)
    {
        if (message.length < 16) {
            message = Buffer.concat([
                message,
                Buffer.alloc(16 - message.length, 0)
            ]);
        }
     ...
     *
     * @param {Buffer} message
     * @return {Promise<module.Poly1305>}
     */
    ...
    async finish()
    {
        let c, f, g0, g1, g2, g3, g4, h0, h1, h2, h3, h4, mask;
        if (this.leftover) {
            let i = this.leftover;
            this.buffer[i++] = 1;
            for (; i < BLOCK_SIZE; i++) {
